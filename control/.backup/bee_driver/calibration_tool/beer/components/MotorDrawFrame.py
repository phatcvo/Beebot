...
class MotorDrawFrame(tb.Frame):
  def __init__(self, parentFrame, motorNo):
    ...
    buttonStyle.configure(buttonStyleName, font=('Arial',10, 'bold'))

    g.motorAngPos[self.motorNo], g.motorAngVel[self.motorNo] = g.serClient.get(f'/data{g.motorLabel[self.motorNo]}')

    self.posText = tb.Label(self.textFrame1, text="POS(rad):", font=('Arial',10, 'bold') ,bootstyle="danger")
    self.posVal = tb.Label(self.textFrame1, text=g.motorAngPos[self.motorNo], font=('Arial',10), bootstyle="dark")

    self.velText = tb.Label(self.textFrame2, text="VEL(rad/s):", font=('Arial',10, 'bold') ,bootstyle="primary")
    self.velVal = tb.Label(self.textFrame2, text=g.motorAngVel[self.motorNo], font=('Arial',10), bootstyle="dark")
