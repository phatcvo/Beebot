...
from beer.globalParams import g, selectSignal


class IMUFrame(tb.Frame):
  def __init__(self, parentFrame, motorNo):
    ...
    buttonStyle.configure(buttonStyleName, font=('Arial',9, 'bold'))


    g.motorTargetVel[self.motorNo], g.motorActualVel[self.motorNo] = g.serClient.get(f'/pVel{g.motorLabel[self.motorNo]}')

    self.actualText = tb.Label(self.textFrame1, text="ACTUAL(rad/s):", font=('Arial',10, 'bold') ,bootstyle="danger")
    self.actualVal = tb.Label(self.textFrame1, text=g.motorActualVel[self.motorNo], font=('Arial',10), bootstyle="dark")

    self.targetText = tb.Label(self.textFrame2, text="TARGET(rad/s):", font=('Arial',10, 'bold') ,bootstyle="primary")
    self.targetVal = tb.Label(self.textFrame2, text=g.motorTargetVel[self.motorNo], font=('Arial',10), bootstyle="dark")
  ...
  def drawGraphicalLine(self):
    ...
    text = self.canvas.create_text(self.xStartOffsetPnt+self.xAxisLen+self.xStopOffsetPnt, (self.h/2)+15,
                                    text="(sec)", fill="green", font=('Arial 7 bold'), angle=90.0)
